...
export class PrivacyConsent {
    ...
    constructor(element: HTMLElement) {
        if (element) {
            ...
            this.acceptButton = element.querySelector(
                ".privacy-consent__accept"
            );
            this.declineButton = element.querySelector(
    ...
    protected get cookieDomain(): string {
        ...
        const cookieDomain = domainParts
            .slice(domainParts.length - 2)
            .join(".");
    ...
    protected addToggleListener() {
        if (this.toggleButton) {
            this.toggleButton.addEventListener("click", () => {
                const pressState = this.toggleButton.getAttribute(
                    "aria-pressed"
                );
                if (pressState === "true") {
                    this.setToggleState(false);
        ...
        if (this.toggleButton) {
            if (toPressed) {
                ...
                this.toggleButton.innerHTML =
                    "Tracking Cookies Enabled, Click to Disable";
            } else {
                this.toggleButton.setAttribute("aria-pressed", "false");
                this.toggleButton.innerHTML =
                    "Tracking Cookies Disabled, Click to Enable";
            }
        }
    }
...
export default function privacyConsentInit() {
    const widgets = document.querySelectorAll(".privacy-consent") as NodeListOf<
        HTMLElement
    >;
